...
export function initFormSolo() {
    ...
    if (btnCharger) {
        ...
        btnCharger.parentNode.replaceChild(clone, btnCharger);
        clone.addEventListener("click", () => {
            const ajouté = ajouterJoueurDepuisListe("liste-joueurs");
            if (ajouté) afficherJoueursSelectionnes("joueurs-selectionnes-container");
        });
    }
    ...
    if (btnCreer) {
        ...
        btnCreer.parentNode.replaceChild(clone, btnCreer);
        clone.addEventListener("click", () => {
    ...
    if (inputPseudo) {
        ...
        inputPseudo.parentNode.replaceChild(clone, inputPseudo);
...
function buildDashboardJoueurHTML(stats, filtre = "tous") {
    ...
    const filtreOptions = [
        ...
        <button class="gj-filtre-btn ${filtre === opt.val ? "active" : ""}" data-filtre="${opt.val}">
    ...
    return `
        <div class="gj-dashboard">
            ...
            <div class="gj-jeux-grid">${cartes || emptyState}</div>
        </div>
    `;
}
...
function renderGestionJoueurs(panel, joueurSelectId = null, filtre = "tous") {
    ...
    panel.querySelector("#gj-btn-ajouter").onclick = () => {
        ...
        if (!pseudo) return;
        if (getPlayers().includes(pseudo)) { alert("Ce joueur existe déjà."); return; }
        addPlayer(pseudo);
    ...
    panel.querySelectorAll(".gj-btn-dashboard").forEach(btn => {
        btn.onclick = () => renderGestionJoueurs(panel, btn.dataset.joueur, "tous");
    });
            ...
            const valider = () => {
                ...
                if (nouveauNom && nouveauNom !== ancienNom) {
                    if (renommerJoueur(ancienNom, nouveauNom)) {
                        renderGestionJoueurs(panel, joueurSelectId === ancienNom ? nouveauNom : joueurSelectId, filtre);
                    } else {
                        renderGestionJoueurs(panel, joueurSelectId, filtre);
                    }
                } else {
    ...
    panel.querySelectorAll(".gj-btn-delete").forEach(btn => {
        btn.onclick = () => {
            ...
            if (supprimerJoueur(pseudo)) {
                renderGestionJoueurs(panel, joueurSelectId === pseudo ? null : joueurSelectId, filtre);
            }
        };
    });

    panel.querySelectorAll(".gj-filtre-btn").forEach(btn => {
        btn.onclick = () => renderGestionJoueurs(panel, joueurSelectId, btn.dataset.filtre);
    });
